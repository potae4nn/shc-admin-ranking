...
export class TableStaffComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.exampleDatabase.getMember('', "userupdate", "desc", 0)
      ...
      ).subscribe(data => this.dataAdmin = data);
  }
  ngAfterViewInit() {
...
export class ExampleHttpDatabase {
  ...
  getMember(dataSearch: string, sort: string, order: SortDirection, page: number): Observable<IMember> {
    return this._userService.getAdmin(dataSearch, page, sort, order)
  }
}
